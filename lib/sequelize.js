...
function createSequelize(_ref) {
  ...
  var _Object$keys$reduce = Object.keys(models).reduce(function (_ref2, name) {
    ...
    var _models$name = models[name],
        ...
        instanceMethods = _models$name.instanceMethods,
        classMethods = _models$name.classMethods;
    ...
    if (cname !== "user") {
      mhook.push.apply(mhook, (0, _user.createUserHooks)(cname, models[name]));
    ...
    if (instanceMethods) {
      Object.keys(instanceMethods).forEach(function (m) {
        global.DB[uname].prototype[m] = instanceMethods[m];
      });
    }

    if (classMethods) {
      Object.keys(classMethods).forEach(function (m) {
