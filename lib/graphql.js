...
function createQueries(sequelize, queries) {
  return function (nodeInterface, resolver) {
    return queries.reduce(function (data, q) {
      ...
      }, _defineProperty(_q, "nodeInterface", nodeInterface), _defineProperty(_q, "sequelizeGraphQLObjectTypes", _relaySequelize.sequelizeGraphQLObjectTypes), _defineProperty(_q, "resolver", resolver), _defineProperty(_q, "GraphQL", GraphQL), _defineProperty(_q, "sequelize", sequelize), _defineProperty(_q, "argsToSequelize", _relaySequelize.argsToSequelize), _defineProperty(_q, "resolveEdge", _relaySequelize.resolveEdge), _defineProperty(_q, "graphqlEdgeTypes", _relaySequelize.graphqlEdgeTypes), _q)));
    }, {});
  };
}
  ...
  return function (nodeInterface, attributeFields) {
    return mutations.reduce(function (data, m) {
      return _extends({}, data, m({
        ...
        graphqlEdgeTypes: _relaySequelize.graphqlEdgeTypes
      }));
    }, {});
  };
}
...
function configureGraphQLServer(app, sequelize, _ref) {
  ...
  app.use(url, (0, _relaySequelize.graphqlExpress)(function (context) {
    var obj = { schema: schema, context: context };
    return obj.context.user ? DB.User.findById(obj.context.user.id).then(function (user) {
